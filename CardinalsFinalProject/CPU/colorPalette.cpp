...
uint compareRGB(uchar blue, uchar green, uchar red)
{
	uint minIndex = 0;
	//initialize minAvg to the first color
	uchar minAvg = (abs(blue - BLUE[0]) + abs(green - GREEN[0]) + abs(red - RED[0])) / 3;
	...
	for (int i = 1; i < NUM_COLORS; i++) //iterate through the array of 143 color codes
	{
		...
		uchar avg = (abs(blue - BLUE[i]) + abs(green- GREEN[i]) + abs(red - RED[i])) / 3;
...
{	//get BGR values from image
	...
	uint* FREQINDEXES = (uint*) malloc(totalPixels * sizeof(uint)); //array for each pixel's closest color index	
	int freqIndexesSize = 0;
	uint colorNameIndex;
	for (int i = 0;  i < totalPixels; i++) {
		colorNameIndex = compareRGB(pixelBArr[i], pixelGArr[i], pixelRArr[i]); //pixel is determined to be closest to the color at this index
		FREQINDEXES[freqIndexesSize] = colorNameIndex; //populate the FREQINDEXES array
		freqIndexesSize++;
	}
	...
	for (int i = 0;  i < NUM_COLORS; i++) { //COLORNAMES (indexes) 
		for(int j = 0; j < freqIndexesSize; j++) { //FREQINDEXES 
			if (i == FREQINDEXES[j]) {
